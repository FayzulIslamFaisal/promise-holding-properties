...
interface ProjectTabViewsProps {
  projects?: Project[];
  currentTab?: string;
  viewType?: 'swiper' | 'grid';
  showTitle?: boolean;
}

const ProjectTabViewsClient = ({
  projects = [],
  currentTab = 'all',
  viewType = 'swiper',
  showTitle = true,
}: ProjectTabViewsProps) => {
  const [activeTab, setActiveTab] = useState(currentTab);

  const filteredProjects = projects.filter((project) => {
    if (activeTab === 'all') return true;
    return project.product_status?.toLowerCase() === activeTab.toLowerCase();
  });

  const [shouldShowNavigation, setShouldShowNavigation] = useState(false);

  useEffect(() => {
    if (viewType !== 'swiper') return;
    ...
    const updateSlides = () => {
      ...
      setShouldShowNavigation(filteredProjects.length > spv);
    };

    updateSlides();
    window.addEventListener('resize', updateSlides);
    return () => window.removeEventListener('resize', updateSlides);
  }, [filteredProjects.length, viewType]);
  ...
  return (
    <section className="w-full sectionSpaceBorder container mx-auto px-4">
      ...
      {/* Tabs */}
      <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full text-white mt-8 md:mt-10">
