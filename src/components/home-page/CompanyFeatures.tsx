...
const CompanyFeatures = () => {
 return (
    <section className="dark:bg-[var(--custom-bg-body)] bg-[var(--custom-bg-white)]/10 py-14 px-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-5">
          {featureItems.map(({ id, icon: Icon, title }) => (
            <div key={id} className="group relative h-full shadow-xl hover:shadow-2xl transition-shadow duration-300 rounded-xl">
              ...
              {/* Main card content */}
              <div className="relative h-full dark:bg-[var(--custom-bg-primary)] bg-[var(--custom-bg-white)] border border-[var(--custom-bg-primary)]/20 dark:border-[var(--custom-bg-accent)]/20 rounded-xl transition-all duration-300 p-6 flex flex-col items-center text-center group-hover:-translate-y-1 z-10 ">
                <div className=" text-[var(--custom-text-white)] rounded-full p-4 mb-4 bg-[var(--custom-bg-primary)] dark:bg-[var(--custom-bg-accent)] shadow-lg group-hover:shadow-xl transition-shadow duration-300">
