...
const TeamMember = async () => {
  ...
  const teamMembers: (ManagementMember | TeamMemberData)[] = (membersRes && membersRes.length) ? membersRes : fallbackMembers;
      ...
      <div className="container mx-auto sectionSpaceBorder">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-4 ">
          {teamMembers.map((member, index: number) => {
            const m = member as ManagementMember & TeamMemberData;
            return (
              <Card
                key={member.id || index}
                variant={"hoverEffect"}
