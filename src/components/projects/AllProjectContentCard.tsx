...
const AllProjectContentCard: React.FC<AllProjectCardProps> = ({ project }) => {
  ...
  return (
    <Link href={`/project/${project.slug}`} className="block h-full cursor-pointer">
      <Card className="group overflow-hidden rounded-2xl !border border-[var(--custom-bg-primary)] shadow-[0_8px_30px_rgba(0,0,0,0.12)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.2)] transition-all duration-300 py-0 gap-0 bg-transparent dark:bg-transparent hover:scale-100 h-full" variant="default">
        <CardContent className="p-0 relative flex flex-col justify-end aspect-[2/3]">
          {/* Background Image */}
          ...
          <div className="relative z-10 p-6 flex flex-col gap-2 transform transition-transform duration-300 group-hover:-translate-y-2">
            ...
            <span className="inline-flex items-center gap-2 text-sm font-semibold text-white/80 group-hover:text-white transition-colors duration-300 w-fit">
